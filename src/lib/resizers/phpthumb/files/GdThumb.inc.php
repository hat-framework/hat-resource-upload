...
class GdThumb extends ThumbBase
{
	...
	public function save ($fileName, $format = null)
	{
		...
		{
			throw new InvalidArgumentException ('Formato de parâmetro invalido para a função save' . $format);
		}
		...
		if (!is_writeable(dirname($fileName)))
		{
			...
			if ($this->options['correctPermissions'] === true)
			{
				...
				if (!is_writeable(dirname($fileName)))
				{
					throw new RuntimeException ('Não existe permissão de escrita para o arquivo ' . $fileName);
				}
			}
			// throw an exception if not writeable
			else
		...
		{
			case 'GIF':
				imagegif($this->oldImage, $fileName . ".gif");
				break;
			case 'JPG':
				imagejpeg($this->oldImage, $fileName . ".jpg", $this->options['jpegQuality']);
				break;
			case 'PNG':
				imagepng($this->oldImage, $fileName . ".png");
				break;
		}
	...
	protected function preserveAlpha ()
	{
        ...
        public function createReflection ($percent, $reflection, $white, $border, $borderColor)
        {
                ...
                $this->newImage			= $this->getOldImage();
