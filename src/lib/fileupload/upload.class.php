...
class UploadHandler extends classes\Classes\Object
{
    protected $options;
    private $upload_folder = "files";
    function __construct($options=null) {
        $folder = array_key_exists('folder', $options)? $options['folder'] . "/":"";
        $this->folder = $folder; 
        $this->relative_path = "$this->upload_folder/$folder";
        ...
        $this->options = array(
            ...
            'image_versions' => array(
                ...
                */
                'thumbnail' => array(
                    'upload_dir' => DIR_FILES.$this->relative_path."thumb/",
        ...
        foreach($this->options['image_versions'] as $arr){
            if(!file_exists($arr['upload_dir'])){
                $this->LoadResource('files/dir', 'dir');
                if(!$this->dir->create($arr['upload_dir'], "")){
                    die($this->dir->getErrorMessage());
                }
            }
        }
     ...
     public function get() {
        ...
        foreach($var as $i => $v){
            $out[$i]['name'] = $v['name'];
            $out[$i]['size'] = "Tamanho: ". arquivoModel::convertSizeUnity($v['size']);
            $out[$i]['ext']  = $v['ext'];
            $out[$i]['url']  = $this->options['script_url']."download.php?file=".$v['cod_arquivo']."&folder=$this->folder";
            if(in_array($v['ext'], $image))
                $out[$i]['thumbnail_url'] = $this->options['upload_url']."thumb/".$v['name'];
            $out[$i]['delete_type'] = "DELETE";
            $out[$i]['delete_url']  = $this->options['script_url']."delete.php?file=".$v['cod_arquivo']."&folder=$this->folder";
